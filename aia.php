<?php include "includes/header.php";?>
<h2 class="pageId"><span class="fa <?=$logo?>"></span><?=$pageId?></h2>
<h3>Client Information</h3>
<table>
    <tr>
        <th scope="row">Client's Name</th>
        <td>Steven Bateman</td>
    </tr>
    <tr>
        <th scope="row">Client's website name</th>
        <td>Zevia’s RMMV Plugins</td>
    </tr>
    <tr>
        <th scope="row">Redesign or brand new website?</th>
        <td>Brand new site</td>
    </tr>
    <tr>
        <th scope="row">Type of website</th>
        <td>Community and Entertainment</td>
    </tr>
    <tr>
        <th scope="row">Client's goal(s)</th>
        <td>The client's goal for their website is collating disparate sources of plugins in an easily searchable manner.</td>
    </tr>
</table>

<h3>Audience, Issues, and Approach Model</h3>
<table>
    <tr>
        <th scope="col">Audiences</th>
        <th scope="col">Issues, Concerns, Needs</th>
        <th scope="col">Approach</th>
    </tr>
    <tr>
        <td>
        Game Players
        <p>10 y/o+, All genders, All income levels, Some primary education, Windows/Mac/Steam/Android/iOS user, 
        Screen resolution 1280x768 or better, Internet bandwidth/speeds vary, Beginner Computer Skills</p>
        </td>
        <td>
            <ul>
                <li></li>
                <li></li>
                <li></li>
            </ul>
        </td>
        <td>
            <ul>
                <li></li>
                <li></li>
                <li></li>
            </ul>
        </td>
    </tr>
    <tr>
        <td>
        Developers
        <p>10 y/o+, All genders, All income levels, Some primary education, Windows/Mac/Steam/Android/iOS user, 
        Internet bandwidth/speeds vary, Screen resolution 1280x768 or better, May need some JavaScript knowledge, 
        Intermediate Computer Skills</p>
        </td>
        <td>
            <ul>
                <li></li>
                <li></li>
                <li></li>
            </ul>
        </td>
        <td>
            <ul>
                <li></li>
                <li></li>
                <li></li>
            </ul>
        </td>
    </tr>
    <tr>
        <td>
        Game Designers
        <p>10 y/o+, all genders, all income levels, Some primary education, Windows/Mac/Steam/Android/iOS user, 
        Internet bandwidth/speeds vary, Screen resolution 1280x768 or better, May need some knowledge on pixel art, 
        spriting, and JavaScript, Intermediate Computer Skills</p>
        </td>
        <td>
            <ul>
                <li></li>
                <li></li>
                <li></li>
            </ul>
        </td>
        <td>
            <ul>
                <li></li>
                <li></li>
                <li></li>
            </ul>
        </td>
    </tr>
</table>
<?php include "includes/footer.php";?>